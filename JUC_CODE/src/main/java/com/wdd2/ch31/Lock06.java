...
/**
 * *题目:谈谈你对多线程锁的理解,8锁案例说明*口诀:线程操作资源类
 * *8锁案例说明:
 ...
 * *7有1个静态同步方法，有1个普通同步方法,有1部手机，请问先打印邮件还是短信
 ...
 * *对于普通同步方法，锁的是当前实例对象，通常指this,具体的一部部手机,所有的普通同步方法用的都是同一把锁—>实例对象本身，*对于静态同步方法，锁的是当前类的CLass对象，如Phone.class唯一的一个模板
 * *对于同步方法块，锁的是synchronized括号内的对象
 * *
 * *7-8
 * 当一个线程试图访问同步代码时它首先必须得到锁，正常退或抛出异常时必须释放锁。
 * ** *所有的普通同步方法用的都是同一把锁—实例对象本身，就是new出来的具体实例对象本身,本类this
 * ***也就是说如果一个实例对象的普通同步方法获取锁后，该实例对象的其他普通同步方法必须等待获取锁的方法释放锁后才能获敬锁，零**
 * ***所有的静态同步方法用的也是同一把锁—类对象本身，就是我们说过的唯一模板cLass
 * ***具体实例对象this和唯一模板class，这两把锁是两个不同的对象，所以静态同步方法与普通同步方法之间是不会有竟态条件的***但是一日一个静态同步方法获取锁后，其他的静态同步方法都必须等待该方法释放锁后才能获取锁。
 *
 */
public class Lock06 {
